...
class GUI(View):
    ...
    def reindex_atoms(self, key, preserve_selection=False):
        atoms = self.atoms

        # Step 1: Record selection
        selected_mask = self.images.selected[:len(atoms)]
        selected_indices = np.array(self.images.selected_ordered)
        unselected_indices = np.where(~selected_mask)[0]

        # Step 2: Compute new atom order
        if key == 'Ctrl+Shift+L':
            # Put selected atoms last
            new_order = np.concatenate([unselected_indices, selected_indices])
        elif key == 'Ctrl+Shift+F':
            # Put selected atoms first
            new_order = np.concatenate([selected_indices, unselected_indices])
        elif key == 'Ctrl+Shift+S':
            # Swap selected atoms
            new_order = np.arange(len(atoms))
            for i, index in enumerate(selected_indices):
                new_i = (i + 1) % len(selected_indices)
                new_index = selected_indices[new_i]
                new_order[new_index] = index
        else:
            return

        # Step 3: Reorder atoms
        self.atoms = atoms[new_order]

        # Step 4: Restore or reset selection
